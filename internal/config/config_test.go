...
func TestTheZeroConfigNamesWhatWorkNeeds(t *testing.T) {
	...
	got, err := c.Claude.StartSession().Render(worktree.Values{"Name": "one", "Dir": "/wt"})
	require.NoError(t, err)
	testenv.Equal(t, []string{"claude", "--permission-mode", "auto", "--name=one"}, got, "the zero Config names no command")
}

...
func New(repo worktree.Repo, from worktree.Path) Resolver { return Resolver{repo: repo, from: from} }

// Named is the resolver a verb reaches by name: it takes the name at its word,
// there being nothing to recognise it against.
func Named(repo worktree.Repo, from worktree.Path) Resolver {
	r := New(repo, from)
	r.own = true
	return r
}
...
func (r Resolver) Identify(id worktree.ID, o worktree.Open) (worktree.Place, error) {
	if o.None() {
		...
		return worktree.Place{ID: id, Name: worktree.Name(id), Branch: worktree.Branch(id)}, nil
	}

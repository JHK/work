...
**Skill.** A set of instructions a coding agent loads on demand, invoked in a session as a slash command. The steps on either side of `work` are usually packaged this way, though which ones exist and what they are called is a matter of how you set your agent up. `--start` hardcodes one name, `/start <id>`, and that is the only one this tool knows.

...
*Enforced by:* `result.came` in `internal/cli/ground_test.go`, which names a refusal whole beside every tool the run asked, so one of work's own is asserted saying nothing a tool said. Each precondition above is read that way in the same package: `repository_test.go` for the first, against a translated git too, `remove_test.go` for the next two, and `move_test.go` and `add_test.go` for the last two. `CommandLine` in `internal/run/run.go` builds the line every other refusal carries.

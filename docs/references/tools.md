...
| [the agent](agent.md), `claude` by default | what a worktree is handed to, and the transcripts under `~/.claude` that say [what an existing one carries](agent.md#the-contract) | the handoff fails, a new worktree having already been created and its bead claimed | `internal/config/command.go`, `internal/sessions/` |
